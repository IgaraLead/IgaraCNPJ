...
class FileDownloader:
    ...
    def __init__(self, max_workers: int = 4, max_retries: int = 3, timeout: int = 300, auth_headers: dict = None):
    ...
    def check_file_needs_download(self, url: str, file_path: str) -> bool:
        ...
        try:
            response = requests.head(url, timeout=self.timeout, headers=self.auth_headers)
    ...
    def _download_with_progress(self, url: str, file_path: str) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                response = session.get(url, stream=True, timeout=self.timeout, headers=self.auth_headers)
                response.raise_for_status()
    ...
    def download_files_parallel(
        ...
        progress_callback(completed_count, filename) is called after each file.
        """
...
class RFBFileDiscovery:
    """
    Discovers RFB files via Nextcloud WebDAV (PROPFIND).
    Downloads use HTTP GET with Basic auth (share_token as user, no password).
    """

    def __init__(self, base_url: str, share_token: str = "gn672Ad4CF8N6TK"):
    ...
    def _webdav_request(self, url: str, method: str = "GET") -> urllib.request.Request:
    ...
    def discover_zip_files(self) -> List[str]:
        """
        Discover available ZIP files via WebDAV PROPFIND.
        Returns list of filenames.
        """
        try:
            self._logger.info(f"Discovering files from {self.base_url}")

            req = self._webdav_request(self.base_url, method="PROPFIND")
            req.add_header("Depth", "1")
            response = urllib.request.urlopen(req, timeout=30)
            xml_data = response.read().decode("utf-8")

            # Parse WebDAV XML to extract .zip filenames
            import re
            href_pattern = r'<d:href>([^<]+\.zip)</d:href>'
            matches = re.findall(href_pattern, xml_data, re.IGNORECASE)

            unique_files = []
            seen = set()
            for href in matches:
                filename = os.path.basename(href)
                if filename and filename.lower().endswith(".zip") and filename not in seen:
                    seen.add(filename)
    ...
    def create_download_tasks(self, filenames: List[str]) -> List[Tuple[str, str]]:
        ...
        URLs use the WebDAV GET endpoint with auth.
        """
        tasks = []
        for filename in filenames:
